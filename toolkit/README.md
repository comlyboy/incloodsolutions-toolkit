...
npm run build      # tsup bundles ESM + CJS, then `tsc --emitDeclarationOnly` emits .d.ts
...
> Declarations are generated by `tsc`, not tsup: this package is on **TypeScript 7**, and
> tsup's bundled `rollup-plugin-dts` does not support it. `dts` is set to `false` in
> [`../shared/tsup-base.config.ts`](../shared/tsup-base.config.ts) and each package's
> `build` script appends `tsc -p tsconfig.json --emitDeclarationOnly`.
